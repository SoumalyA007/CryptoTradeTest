...
import com.soumalya.CryptoTrade.Repository.CoinRepository;
...
//        coinRepository.save(coins);
    ...
    @Override
    public String getPrediction(String coinId, int days) {
        // Allowed values for 'days'
        List<Integer> allowedDays = Arrays.asList(30, 90, 180, 330);

        // Validate user input
        if (!allowedDays.contains(days)) {
            return "{\"error\": \"Invalid days value. Allowed values are 30, 90, 180, 330.\"}";
        }

        // Construct the API URL
        String url = "https://cryptopredict-fupd.onrender.com/predict/?coin=" + coinId + "&days=" + days;

        RestTemplate restTemplate = new RestTemplate();
        try {
            HttpEntity<String> entity = new HttpEntity<>(null); // No headers needed
            ResponseEntity<String> response = restTemplate.exchange(url, HttpMethod.GET, entity, String.class);

            return response.getBody();

        } catch (HttpClientErrorException | HttpServerErrorException e) {
            System.err.println("Error calling prediction API: " + e);
            return "{\"error\": \"Prediction service is unavailable\"}";
        }
    }
